...
public abstract class EgovComAbstractDAO extends EgovAbstractMapper{
	...
	public <E> List<E> selectList(String queryId, Object parameterObject) {
		...
		/*String sql = getSqlSession().getConfiguration().getMappedStatement(queryId).getBoundSql(parameterObject).getSql();
		List<ParameterMapping> paramList = getSqlSession().getConfiguration().getMappedStatement(queryId).getBoundSql(parameterObject).getParameterMappings();
		for(ParameterMapping param : paramList){
			LOGGER.debug("param : "+param.getProperty());
		}

		LOGGER.debug("select query log : "+sql);*/
